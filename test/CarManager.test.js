...
contract("CarManager", (accounts) => {
  ...
  beforeEach(async () => {
    ...
    authorizerContract.addPermission(carManagerContract.address, CREATE_CAR_METHOD, manufacturerAddress);

    authorizerContract.addPermission(carManagerContract.address, DELIVER_CAR_METHOD, manufacturerAddress);

    authorizerContract.addPermission(carManagerContract.address, REGISTER_CAR_METHOD, brandAddress);

    authorizerContract.addPermission(carManagerContract.address, SELL_CAR_METHOD, brandAddress);

    authorizerContract.addPermission(carManagerContract.address, SELL_CAR_METHOD, brandAddress);

    authorizerContract.addPermission(carManagerContract.address, SELL_CAR_METHOD, brand2Address);
  });
  ...
  it("Add new car from manufacturer account", async () => {
    ...
    const events = await carManagerContract.getPastEvents("allEvents", {
      filter: {
  ...
  it("Add new car from invalid account", async () => {
    try {
      ...
      assert.include(e.message, REVERT_ERROR);
    }
  });
  ...
  it("Deliver car from manufacturer account", async () => {
    ...
    await carManagerContract.deliverCar(carAddress, {
      from: manufacturerAddress,
    });

    await carAssetContract.transferFrom(manufacturerAddress, brandAddress, carIdHash, {
      from: manufacturerAddress,
    });
  ...
  it("Deliver car from no owner asset brand account", async () => {
    ...
    try {
      await carAssetContract.transferFrom(brandAddress, userAddress, carIdHash, {
        from: manufacturer2Address,
      });
    } catch (e) {
  ...
  it("Register car from the brand account", async () => {
    ...
    await carManagerContract.sellCar(carAddress, {
      from: brandAddress,
    });

    await carManagerContract.registerCar(carAddress, licensePlate, {
      from: brandAddress,
    });
  ...
  async function createCar(from) {
    await carManagerContract.createCar(carIdHash, carIdHashSigned.signature, carTypes.THREE_WHEEL, {
      from,
    });
  }
  ...
  async function createCarAndDeliver(manufacturer, seller) {
    ...
    await carAssetContract.transferFrom(manufacturer, seller, carIdHash, {
      from: manufacturer,
    });
  }
});
